...
public class ListAppFragment extends VFragment<ListAppContract.ListAppPresenter> implements ListAppContract.ListAppView {
    ...
    /**
     * Filter the displayed app list by name or package name.
     */
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (path == null) {
            // Fallback: copy content URI to cache
            path = copyUriToCache(getActivity(), uri);
        }
        if (path == null) {
            Toast.makeText(getActivity(), "Unable to access the file", Toast.LENGTH_SHORT).show();
            return;
        }
