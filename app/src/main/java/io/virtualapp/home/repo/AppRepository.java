...
public class AppRepository implements AppDataSource {
    ...
    private static boolean isSystemApplication(PackageInfo packageInfo) {
        if (GmsSupport.isGmsFamilyPackage(packageInfo.packageName)) {
            return false; // Always show GMS packages
        }
        boolean isSystem = (packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0;
        if (!isSystem) {
            return false; // Not a system app
        }
        // System apps with an updated version (user-updated) should be shown
    ...
    public Promise<List<AppInfo>, Throwable, Void> getInstalledApps(Context context) {
        return VUiKit.defer().when(() -> {
            ...
            try {
                allPackages = context.getPackageManager().getInstalledPackages(0);
            } catch (Exception e) {
                allPackages = new ArrayList<>();
            }
            return convertPackageInfoToAppData(context, allPackages, true);
        });
    }
