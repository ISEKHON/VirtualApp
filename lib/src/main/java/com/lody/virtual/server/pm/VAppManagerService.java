...
public class VAppManagerService extends IAppManager.Stub {
    ...
    private boolean loadPackageInnerLocked(PackageSetting ps) {
        ...
        try {
            ...
            VLog.w(TAG, "readPackageCache failed for " + ps.packageName, e);
        }
        if (pkg == null || pkg.packageName == null) {
            // Cache is invalid/corrupt. Try to re-parse from the APK file instead of deleting.
            VLog.w(TAG, "Package cache invalid for " + ps.packageName + ", attempting re-parse from APK");
            File apkFile = ps.apkPath != null ? new File(ps.apkPath) : null;
            if (apkFile == null || !apkFile.exists()) {
                // Try the default location
                apkFile = new File(VEnvironment.getDataAppPackageDirectory(ps.packageName), "base.apk");
            }
            if (apkFile.exists()) {
                try {
                    pkg = PackageParserEx.parsePackage(apkFile);
                    if (pkg != null && pkg.packageName != null) {
                        VLog.i(TAG, "Successfully re-parsed " + ps.packageName + " from " + apkFile.getPath());
                        // Save the cache so it works next time
                        PackageParserEx.savePackageCache(pkg);
                    }
                } catch (Throwable e2) {
                    VLog.e(TAG, "Re-parse also failed for " + ps.packageName, e2);
                }
            }
            if (pkg == null || pkg.packageName == null) {
                VLog.e(TAG, "Cannot recover package " + ps.packageName + ", will clean up");
                return false;
            }
        }
    ...
    public synchronized InstallResult installPackage(String path, int flags, boolean notify) {
        ...
        // If path is a directory (split APK), look for base.apk inside
        if (packageFile.isDirectory()) {
            File baseApk = new File(packageFile, "base.apk");
            if (!baseApk.exists()) {
                // Try to find any APK file in the directory
                File[] apks = packageFile.listFiles((dir, name) -> name.toLowerCase().endsWith(".apk"));
                if (apks != null && apks.length > 0) {
                    // Use the directory itself for PackageParser (it handles split APK dirs)
                    // packageFile stays as directory
                } else {
                    return InstallResult.makeFailure("No APK found in directory: " + path);
                }
            }
        }
        VPackage pkg = null;
        try {
            ...
            VLog.e(TAG, "parsePackage failed for: " + path, e);
            e.printStackTrace();
        }
        if (pkg == null || pkg.packageName == null) {
            return InstallResult.makeFailure("Unable to parse the package: " + path);
        }
