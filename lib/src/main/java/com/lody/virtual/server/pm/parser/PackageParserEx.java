...
public class PackageParserEx {
    ...
    private static void buildSignature(PackageParser.Package p, Signature[] signatures) {
        if (Build.VERSION.SDK_INT < 28) {
            ...
            try {
                ...
                if (mirror.android.content.pm.PackageParser.Package.mSigningDetails != null) {
                    signingDetails = mirror.android.content.pm.PackageParser.Package.mSigningDetails.get(p);
                }
                if (signingDetails != null) {
                    if (mirror.android.content.pm.PackageParser.SigningDetails.pastSigningCertificates != null) {
                        mirror.android.content.pm.PackageParser.SigningDetails.pastSigningCertificates.set(signingDetails, signatures);
                    }
                    if (mirror.android.content.pm.PackageParser.SigningDetails.signatures != null) {
                        mirror.android.content.pm.PackageParser.SigningDetails.signatures.set(signingDetails, signatures);
                    }
                }
            } catch (Throwable e) {
                VLog.w(TAG, "buildSignature: failed to set signing details via mirror", e);
            }
        }
    }
    ...
    private static VPackage buildPackageCache(PackageParser.Package p) {
        ...
        if (mirror.android.content.pm.PackageParser.Package.protectedBroadcasts != null) {
            try {
                // On Android 14+, protectedBroadcasts may be ArraySet instead of List.
                // Use raw Object access to avoid ClassCastException.
                Object rawBroadcasts = mirror.android.content.pm.PackageParser.Package.protectedBroadcasts.get(p);
                if (rawBroadcasts instanceof java.util.Collection) {
                    java.util.Collection<String> broadcasts = (java.util.Collection<String>) rawBroadcasts;
                    cache.protectedBroadcasts = new ArrayList<>(broadcasts);
                }
            } catch (Throwable e) {
                VLog.w(TAG, "Failed to read protectedBroadcasts (non-fatal)", e);
            }
        }
        ...
        if (Build.VERSION.SDK_INT < 28) {
            ...
            try {
                ...
                if (signingDetails != null) {
                    // Extract signatures from SigningDetails
                    boolean hasPastCerts = false;
                    if (mirror.android.content.pm.PackageParser.SigningDetails.hasPastSigningCertificates != null) {
                        hasPastCerts = Boolean.TRUE.equals(
                                mirror.android.content.pm.PackageParser.SigningDetails.hasPastSigningCertificates.call(signingDetails));
                    }
                    if (hasPastCerts && mirror.android.content.pm.PackageParser.SigningDetails.pastSigningCertificates != null) {
                        Signature[] pastSigs = mirror.android.content.pm.PackageParser.SigningDetails.pastSigningCertificates.get(signingDetails);
                        if (pastSigs != null && pastSigs.length > 0) {
                            cache.mSignatures = new Signature[]{pastSigs[0]};
                        }
                    } else {
                        boolean hasSigs = false;
                        if (mirror.android.content.pm.PackageParser.SigningDetails.hasSignatures != null) {
                            hasSigs = Boolean.TRUE.equals(
                                    mirror.android.content.pm.PackageParser.SigningDetails.hasSignatures.call(signingDetails));
                        }
                        if (hasSigs && mirror.android.content.pm.PackageParser.SigningDetails.signatures != null) {
                            Signature[] signatures = mirror.android.content.pm.PackageParser.SigningDetails.signatures.get(signingDetails);
                            if (signatures != null && signatures.length > 0) {
                                cache.mSignatures = new Signature[signatures.length];
                                System.arraycopy(signatures, 0, cache.mSignatures, 0, signatures.length);
                            }
                        }
                    }
                    // Create SigningInfo - ctor may be null on Android 14+ where PackageParser$SigningDetails changed
                    if (mirror.android.content.pm.PackageParser.SigningInfo.ctor != null) {
                        cache.signingInfo = mirror.android.content.pm.PackageParser.SigningInfo.ctor.newInstance(signingDetails);
                    } else {
                        // Try direct reflection: find a SigningInfo constructor that accepts signingDetails
                        try {
                            java.lang.reflect.Constructor<?>[] ctors = android.content.pm.SigningInfo.class.getDeclaredConstructors();
                            for (java.lang.reflect.Constructor<?> c : ctors) {
                                Class<?>[] params = c.getParameterTypes();
                                if (params.length == 1 && params[0].isInstance(signingDetails)) {
                                    c.setAccessible(true);
                                    cache.signingInfo = (android.content.pm.SigningInfo) c.newInstance(signingDetails);
                                    break;
                                }
                            }
                        } catch (Exception ignored) {
                            VLog.w(TAG, "Could not create SigningInfo via reflection");
                        }
                    }
                }
            } catch (Throwable e) {
        ...
        cache.usesLibraries = p.usesLibraries != null ? new ArrayList<>(p.usesLibraries) : null;
        ...
        cache.usesOptionalLibraries = p.usesOptionalLibraries != null ? new ArrayList<>(p.usesOptionalLibraries) : null;
