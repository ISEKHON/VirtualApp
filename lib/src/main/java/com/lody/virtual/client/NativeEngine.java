...
public class NativeEngine {
    ...
    public static void enableIORedirect() {
        try {
            ...
            if (!new File(soPath).exists()) {
                VLog.w(TAG, "Native library not found at " + soPath + ", skipping IO redirect");
                return;
            }
            redirectDirectory(VESCAPE, "/");
            nativeEnableIORedirect(soPath, Build.VERSION.SDK_INT, BuildCompat.getPreviewSDKInt());
            VLog.i(TAG, "Native IO redirect enabled successfully on API " + Build.VERSION.SDK_INT);
        } catch (Throwable e) {
            VLog.e(TAG, "enableIORedirect failed (non-fatal): " + VLog.getStackTraceString(e));
        }
    }
    ...
    static void launchEngine() {
        ...
        try {
            ...
            VLog.i(TAG, "Native engine launched successfully on API " + Build.VERSION.SDK_INT);
        } catch (Throwable e) {
            VLog.e(TAG, "launchEngine failed (non-fatal): " + VLog.getStackTraceString(e));
        }
        sFlag = true;
    }
