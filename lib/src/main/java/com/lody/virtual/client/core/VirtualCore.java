...
public final class VirtualCore {
    ...
    public void startup(Context context) throws Throwable {
        if (!isStartUp) {
            ...
            // Try Android 14-compatible bypass first, then legacy approach
            boolean bypassed = false;
            try {
                bypassed = HiddenApiBypass.exemptAll();
                Log.i("VirtualCore", "HiddenApiBypass.exemptAll() = " + bypassed);
            } catch (Throwable e) {
                Log.w("VirtualCore", "HiddenApiBypass failed", e);
            }
            if (!bypassed) {
                try {
                    Reflection.unseal(context);
                    Log.i("VirtualCore", "Reflection.unseal() succeeded");
                } catch (Throwable e) {
                    Log.w("VirtualCore", "Reflection.unseal() failed", e);
                }
            }
