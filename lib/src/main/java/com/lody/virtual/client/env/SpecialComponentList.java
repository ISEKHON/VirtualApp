...
public final class SpecialComponentList {
    ...
    public static void protectIntentFilter(IntentFilter filter) {
        if (filter != null) {
            // On Android 14+, IntentFilter.mActions may be ArraySet instead of ArrayList.
            // Use raw Object access to handle both types safely.
            Object rawActions = mirror.android.content.IntentFilter.mActions.get(filter);
            if (rawActions instanceof List) {
                List<String> actions = (List<String>) rawActions;
            ...
            } else if (rawActions instanceof java.util.Collection) {
                // ArraySet or other Collection that doesn't support ListIterator.
                // Build a new list, modify, and replace the mActions field.
                java.util.Collection<String> actionsSet = (java.util.Collection<String>) rawActions;
                ...
                for (String action : actionsSet) {
                    ...
                    String newAction = SpecialComponentList.protectAction(action);
                    newActions.add(newAction != null ? newAction : action);
                }
                // Replace the actions collection with our modified ArrayList
                try {
                    java.lang.reflect.Field mActionsField = IntentFilter.class.getDeclaredField("mActions");
                    mActionsField.setAccessible(true);
                    mActionsField.set(filter, newActions);
                } catch (Throwable e) {
                    // Fallback: clear and re-add to original
                    actionsSet.clear();
                    actionsSet.addAll(newActions);
                }
            }
        }
    }
