...
public class NativeLibraryHelperCompat {
	...
	private static Set<String> getABIsFromApk(String apk) {
		// Handle split APK directories: path may be a directory containing base.apk + split APKs
		File apkPath = new File(apk);
		if (apkPath.isDirectory()) {
			Set<String> allABIs = new HashSet<>();
			File[] apkFiles = apkPath.listFiles((dir, name) -> name.endsWith(".apk"));
			if (apkFiles != null) {
				for (File f : apkFiles) {
					Set<String> abis = getABIsFromSingleApk(f.getAbsolutePath());
					if (abis != null) {
						allABIs.addAll(abis);
					}
				}
			}
			return allABIs.isEmpty() ? null : allABIs;
		}
		return getABIsFromSingleApk(apk);
	}

	private static Set<String> getABIsFromSingleApk(String apk) {
